...
public class TimesOrderedByDish extends Thread {
    private String ip;
    private int port;
    private OrderedTimes[] orderedTimes;
    public TimesOrderedByDish(String ip,int port) {
        this.ip=ip;
        this.port=port;
    }
    public OrderedTimes[] getOrderedTimes() {
        return orderedTimes;
    }
    public void run(){
        Socket s=null;
        try{
            ...
            dos.writeUTF("TimesOrderedByDish");
            dos.writeUTF(Variables.currentUser);
            orderedTimes=new Gson().fromJson(dis.readUTF(),OrderedTimes[].class);
        } catch (Exception e) {
            e.printStackTrace();
        } finally{
            try {
